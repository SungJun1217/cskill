...
## Shared scratchpad (the handoff medium)

The moment you hand work from one spawned agent to another — planner → coder, coder → reviewer, explorer → planner — the receiving agent is spawned fresh and does NOT see this conversation. `.claude/run/notes.md` is how state crosses that gap, so create it whenever a handoff will happen, not only for big fan-outs. Start such a task by resetting it: `mkdir -p .claude/run`, then write a one-line task header (overwriting any stale file left from a previous task), and tell each spawned agent the file exists. Agents read prior findings from it and append their own — the planner writes its plan there as the coder's brief, the coder appends what it changed and the intended behavior for the reviewer. Keep it terse — locations (`path:line`), contracts, working commands, intended behavior. It's a per-task scratch file (gitignored), not durable memory; for a single inline edit with no handoff, skip it entirely. Durable cross-session knowledge still goes to a skill via `/cskill:skill-save`.
