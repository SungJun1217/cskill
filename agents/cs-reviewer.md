...
**Shared scratchpad — read it first:** If `.claude/run/notes.md` exists, read it before reviewing — when you were spawned from a coder, it holds the intended behavior, what changed (`path:line`), and the contracts earlier agents recorded. Review against that intent, not your own convention. Don't write findings there; return them as your output.
